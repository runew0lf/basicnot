...
Pagination added by runew0lf#0001"""
...
class Help((commands.DefaultHelpCommand)):
    ...
    def __init__(self, bot: AutoShardedBot, *args, **kwargs):
        self.bot = bot
        global orig_help
    ...
    async def send(self, dest, content=None, embeds=None):
        ...
        def event_check(reaction_: Reaction, user_: Member):
            ...
            return (
                ...
                and reaction_.emoji  # Reaction on this specific message
                in PAGINATION_EMOJI
                and user_.id != self.bot.user.id  # One of the reactions we handle
            )
        ...
        while True:
            try:
                reaction, user = await self.bot.wait_for(
                    "reaction_add", timeout=300, check=event_check
                )
            ...
            if reaction.emoji == FIRST_EMOJI:
                ...
                current_page = 0
                await help_msg.edit(content=content, embed=embeds[current_page])
    ...
    async def format_help_for(self, ctx, command_or_bot, reason: str = None):
        ...
        for field in emb["fields"]:
            ...
            if len(txt) > MAXLEN:
                ...
                embed = discord.Embed(color=self.color, **emb["embed"])
                embed.set_author(
                    name=f"{self.bot.user.name} Help Manual Page {len(embeds)+1}",
                    icon_url=self.avatar,
                )
                embed.set_footer(**emb["footer"])
    ...
    async def help(self, ctx, *cmds: str):
        if not ctx.message.author.permissions_in(ctx.channel).embed_links:
            return await ctx.send(
                content="You don't have permissions to send embeds here. Find a different server/channel where you can embed links and try the help command there."
            )
